...
function SDateTimeMask(textId, formatString) {
	...
	this.maskString = formatString || SDateTimeMaskConfig.defaultFormatString;

	this.initMask( formatString || SDateTimeMaskConfig.defaultFormatString );
	this.initEvent();
...
SDateTimeMask.prototype.initEvent = function () {
	...
	this.$ele.bind("blur.datemask", $.proxy(this._onBlur, this));
};
...
SDateTimeMask.prototype.initMask = function ( formatString ) {
	...
	if (   ( formatString.indexOf("y") >= 0 && formatString.indexOf("y") != formatString.lastIndexOf("y") - 3 )
		|| ( formatString.indexOf("M") >= 0 && formatString.indexOf("M") != formatString.lastIndexOf("M") - 1 )
		|| ( formatString.indexOf("d") >= 0 && formatString.indexOf("d") != formatString.lastIndexOf("d") - 1 )
		|| ( formatString.indexOf("h") >= 0 && formatString.indexOf("h") != formatString.lastIndexOf("h") - 1 )
		|| ( formatString.indexOf("m") >= 0 && formatString.indexOf("m") != formatString.lastIndexOf("m") - 1 )
		|| ( formatString.indexOf("s") >= 0 && formatString.indexOf("s") != formatString.lastIndexOf("s") - 1 ) ) {
		formatString = SDateTimeMaskConfig.defaultFormatString;
	}
...
SDateTimeMask.prototype.inputBackSpaceInArea = SDateTimeMask.prototype.inputDeleteInArea;
...
SDateTimeMask.prototype.isKeyValid = function ( pressKeyCode ) {
	...
	if ( SDateTimeMaskConfig.WORK_KEY == null || SDateTimeMaskConfig.WORK_KEY.length == 0 ) {
		return false;
	}
...
SDateTimeMask.prototype.initBlur = function () {
	this.$ele.bind("blur.datemask", $.proxy(this._onBlur, this));
}
